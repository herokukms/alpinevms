...
RUN apk add --no-cache git make build-base libcurl curl-dev openssl-dev && \
    ...
    pwd && ls -l && \ 
    VERBOSE=1 CC=gcc CFLAGS="-DUSE_THREADS -DLOG_TO_MONGODB -DFULL_INTERNAL_DATA -D_GNU_SOURCE" LDFLAGS="-lpthread -lssl -lcrypto -lcurl   "  GETIFADDRS=musl DNS_PARSER=internal make && \
    ls -l && pwd 
